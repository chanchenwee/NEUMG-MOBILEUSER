<template>
	<div style="background-color:#F7F7F7">
		<div class="thishead">
			<van-nav-bar class="centerhead" title="领券中心" left-arrow @click-left="onClickLeft" @click-right="showPopup">
				<template #right>
		...
		<div class="img">
			<van-swipe :autoplay="3000">
				<van-swipe-item>
					<img style="border-radius:7px;" width="95%" height="150px" src="../../../public/imgs/couponcenter/coupon1.jpg" />
				</van-swipe-item>
				<van-swipe-item>
					<img style="border-radius:7px;" width="95%" height="150px" src="../../../public/imgs/couponcenter/couponcenter1.jpg" />
				</van-swipe-item>
			</van-swipe>
		...
		<div class="coupons">
			<template v-for="coupon in coupons">
				<div :class="CouponStatus(coupon.couponid)">
					<div class="van-coupon__content">
						<div class="van-coupon__head">
							<h2 class="van-coupon__amount">{{coupon.couponscore}}<span>元</span></h2>
						...
						<div class="van-coupon__body">
							...
							<div role="checkbox" tabindex="0" aria-checked="false" class="van-checkbox van-coupon__corner" size="18">
								<van-button v-if="buttext[coupon.couponid]" type="danger" round class="but" @click="getCoupon($event,coupon)">领取</van-button>
							<p v-if="!buttext[coupon.couponid]"class="alreadyhave"></p>
							</div>
						</div>
					</div>
		<!-- 			<p class="van-coupon__description">{{coupon.coupondesc}}</p> -->
			  ...
			  init(){
			    ...
			    this.axios.get(`${server}${getAll}`).then((res) => {
			               ...
			               for(let i=0;i<this.coupons.length;i++){
			                   ...
			                   coupon.couponscore=numArr[1];//拿到满减的金额数目
							   coupon.couponname="满"+numArr[0]+"元可用";
							   this.activeName[coupon.couponid]=i;
